...
await writeFile('public/favicon.svg', icon.replace('</svg>', '<style>@media(prefers-color-scheme:dark){g[fill]{fill:#f3f0e8}}</style></svg>'));
await sharp(Buffer.from(icon)).resize(32, 32).png().toFile('public/assets/brand/favicon-32.png');
await sharp(Buffer.from(svg(180, 180, `<rect width="180" height="180" fill="${paper}"/><g transform="translate(24.4 35.1) scale(3.35)">${mark}</g>`))).png().toFile('public/assets/brand/apple-touch-icon.png');
