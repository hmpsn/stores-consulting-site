...
const hidden = (name: string, type: 'string' | 'number' = 'string'): TinaField => (type === 'number' ? {name,type:'number',ui:{component:null}} : {name,type:'string',ui:{component:null}});
const text = (name: string, label: string, required = true) => ({name, label, type: 'string' as const, required});
const body: TinaField = {name: 'body', label: 'Body (Markdown)', type: 'string', isBody: true, ui: {component: 'textarea'}, description: 'Edit text and Markdown links. Preserve existing image markup; use the image fields for new featured images.'};
...
export const editorialCollections: Collection[] = [
 ...
 {name:'author',label:'Blog authors',path:'src/content/authors',format:'yaml',ui:{...existing,router:({document}:any)=>document.route},fields:[{...text('name','Name'),isTitle:true},hidden('slug'),hidden('route'),{...text('description','Description',false),ui:{component:'textarea'}},{name:'avatar',label:'Avatar',type:'image'},hidden('sourceId','number')]},
];
